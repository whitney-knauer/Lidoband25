...
This will describe the health scores of calves across treatment groups and days. Need to do look at Study ID: 1 3 53 57 62 63 for data discrepancies. Will figure those out once all the data is pulled in.
...
#         0  1
#   BO   77 11 = 11/88=12.5
#   B+N  68 21 = 21/89=23.6
#   LIDO 78 12 = 12/90=13.3

#worth evaluating with models? Only n=6 calves were ID'd with abnormal FS
CrossTable(hsfull$Scour,  hsfull$Calf.Age, fisher=TRUE) #more likely to have loose fecal score on D1 and D2 (likely due to colostrum feeding)
CrossTable(hsfull$Scour,  hsfull$TRT, fisher = TRUE) #no
...
CrossTable(hsfull$Sick,  hsfull$Calf.Age, fisher=TRUE) #fewer on d4 vs. 1 - 3 which is likly a function of when during day they were evaluated? 
CrossTable(hsfull$Sick,  hsfull$TRT, fisher = TRUE) #no
CrossTable(hsfull$Sick, hsfull$Egroup, fisher = TRUE) #no, numerically more in the second iteration which makes sense bc it was ungodly hot. 
```
...
lm_TFAge <- lm(Tail.flick ~ AgeatEnroll, data=ethofull)
summary(lm_TFAge)

lm_TFEG <- lm(Tail.flick ~ Egroup, data=ethofull)
summary(lm_TFEG)

lm_TFBW <- lm(Tail.flick ~ BWKg, data=ethofull)
summary(lm_TFBW)
...
lm_KickAge <- lm(Kick ~ AgeatEnroll, data=ethofull)
summary(lm_KickAge)

lm_KickEG <- lm(Kick ~ Egroup, data=ethofull)
summary(lm_KickEG)

lm_KickBW <- lm(Kick ~ BWKg, data=ethofull)
summary(lm_KickBW)

lm_Kick <- lm(Kick ~ TRT + BWKg, data=ethofull)
...
table(ethofull$TRT, ethofull$Vocalize)
```
...
table(ethofull$TRT, ethofull$Layed.Down)
