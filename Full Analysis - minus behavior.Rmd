...
model <- glmer(Fever ~ TRT + (1 + Calf.Age | CalfID), family = "binomial", data = hsfull)
...
lm_s <- lm(Struggle ~ TRT, data=ethofull)
summary(lm_s)

lm_sAge <- lm(Struggle ~ AgeatEnroll, data=ethofull)
summary(lm_sAge)

lm_sEG <- lm(Struggle ~ Egroup, data=ethofull)
summary(lm_sEG)

lm_sBW <- lm(Struggle ~ BWKg, data=ethofull)
summary(lm_sBW)
...
#What if we get rid of outlier in the data (n>2SD from the mean) Makes it look a lot better, but this is lying with the data. What about a transformation? 

noout<- 
  enroll %>% 
      filter (!HbpostmgdL>29.2)
...
log<- 
  enroll %>% 
  mutate(
    HbpostmgdL = log10(HbpostmgdL), 
    HbpremgdL = log10(HbpremgdL))
      

ggplot(log, aes(x = TRT, y = HbpremgdL, color = TRT)) +
  geom_boxplot() +
  geom_jitter() +
    theme_classic()

ggplot(log, aes(x = HbpremgdL)) +
  geom_histogram() +
    theme_classic()


ggplot(log, aes(x = TRT, y = HbpostmgdL, color = TRT)) +
  geom_boxplot() +
  geom_jitter() +
    theme_classic()
  
ggplot(log, aes(x = HbpostmgdL)) +
  geom_histogram() +
    theme_classic()
