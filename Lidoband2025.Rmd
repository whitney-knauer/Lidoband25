...
key<- read_csv("data/calfkey thirty.csv")
...
#want to create a varaible called cattime that includes Day 1 (1 - 24), Day 2, etc. 
hour <- hour |> 
  mutate(timecat = cut(hour, 
...
hourrm$LyingTime_num <- as.numeric(hourrm$LyingTime, units = "hours")
lm_foo <- lm(LyingTime_num ~ FOO, data = hourrm)
summary(lm_foo)


hourrm$LyingTime_num <- as.numeric(hourrm$LyingTime, units = "hours")
lm_age <- lm(LyingTime_num ~ AgeatEnroll, data = hourrm)
summary(lm_age) 

hourrm$LyingTime_num <- as.numeric(hourrm$LyingTime, units = "hours")
lm_group <- lm(LyingTime_num ~ Egroup, data = hourrm)
summary(lm_group)
...
mod_ar1 <- lme(
  fixed = LyingTime ~ hour * TRT + AgeatEnroll + Egroup,
  random = ~ 1 | Cow,
  correlation = corAR1(form = ~ TimeNum | Cow),
  data = hourrm)
...
intercept_24 <- which(levels(emm_df$hour) == "24")
intercept_48<- which(levels(emm_df$hour) == "48")
custom_colors <- c("BO" = "red4", "B+N" = "goldenrod2", "LIDO" = "wheat4")

ggplot(emm_df, aes(x = hour, y = emmean, color = TRT, group = TRT)) +
     geom_vline(xintercept = c(intercept_24, intercept_48), linetype = "dashed", color = "gray11")+
  geom_line() +
  geom_point() +
...
#Analysis by time category like in other analysis: 

lm_lay <-lmer(as.numeric(LyingTime) ~TRT*timecat + AgeatEnroll + Egroup + (1|Cow), data=hourrm)
summary(lm_lay)
...
hourrm$Stand_num <- as.numeric(hourrm$StandingTime, units = "hours")
lm_foo <- lm(Stand_num ~ FOO, data = hourrm)
summary(lm_foo)


hourrm$Stand_num <- as.numeric(hourrm$StandingTime, units = "hours")
lm_age <- lm(Stand_num ~ AgeatEnroll, data = hourrm)
summary(lm_age) 

hourrm$Stand_num <- as.numeric(hourrm$StandingTime, units = "hours")
lm_group <- lm(Stand_num ~ Egroup, data = hourrm)
summary(lm_group)
...
mod_ar1 <- lme(
  fixed = StandingTime ~ hour * TRT + AgeatEnroll + Egroup,
  random = ~ 1 | Cow,
  correlation = corAR1(form = ~ TimeNum | Cow),
  data = hourrm
)
...
lm_std <-lmer(as.numeric(StandingTime) ~TRT*timecat + AgeatEnroll + Egroup + (1|Cow), data=hourrm)
summary(lm_std)
emmeans(lm_std, ~TRT|timecat)
anova(lm_std, type = 3)
plot(lm_std)


emm_std <- emmeans(lm_std, ~ TRT | timecat)
contrast_results <- pairs(emm_std, reverse = FALSE)
summary(contrast_results, adjust = "tukey")
...
lm_foo <- lm(LyingBouts ~ FOO, data = hourrm)
summary(lm_foo)

lm_age <- lm(LyingBouts ~ AgeatEnroll, data = hourrm)
summary(lm_age) 

lm_group <- lm(LyingBouts ~ Egroup, data = hourrm)
summary(lm_group)
...
mod_ar1 <- lme(
  fixed = LyingBouts ~ hour * TRT + Egroup + AgeatEnroll + FOO,
  random = ~ 1 | Cow,
  correlation = corAR1(form = ~ TimeNum | Cow),
  data = hourrm
)
...
comp <- emmeans(mod_ar1, pairwise ~ TRT, adjust = "tukey")
comp$contrasts
...
lm_lb <-lmer(as.numeric(LyingBouts) ~TRT*timecat + AgeatEnroll + Egroup + (1|Cow), data=hourrm)
summary(lm_lb)
...
emm_lb <- emmeans(lm_lb, ~ TRT | timecat)
contrast_results <- pairs(emm_lb, reverse = FALSE)
summary(contrast_results, adjust = "tukey")
...
lm_foo <- lm(Steps ~ FOO, data = hourrm)
summary(lm_foo)

lm_age <- lm(Steps ~ AgeatEnroll, data = hourrm)
summary(lm_age) 

lm_group <- lm(Steps ~ Egroup, data = hourrm)
summary(lm_group)
...
lm_st <-lmer(as.numeric(Steps) ~TRT*timecat + AgeatEnroll + Egroup + (1|Cow), data=hourrm)
summary(lm_st)
...
emm_st <- emmeans(lm_st, ~ TRT | timecat)
contrast_results <- pairs(emm_st, reverse = FALSE)
summary(contrast_results, adjust = "tukey")
...
Is this a better measure of restlessness? I am unsure but I am also unsure about Steps.
...
lm_foo <- lm(MotionIndex ~ FOO, data = hourrm)
summary(lm_foo)

lm_age <- lm(MotionIndex ~ AgeatEnroll, data = hourrm)
summary(lm_age) 

lm_group <- lm(MotionIndex ~ Egroup, data = hourrm)#no
summary(lm_group)
...
mod_ar1 <- lme(
  fixed = MotionIndex ~ hour * TRT + FOO + AgeatEnroll,
  random = ~ 1 | Cow,
  correlation = corAR1(form = ~ TimeNum | Cow),
  data = hourrm
)
...
ggplot(emm_df, aes(x = hour, y = emmean, color = TRT, group = TRT)) +
 geom_vline(xintercept = c(intercept_24, intercept_48), linetype = "dashed", color = "gray11")+  
  ...
  labs(
    x = "Hour from Banding",
    y = "Estimated Marginal Means of Motion Index") +
  theme_classic()+
  scale_y_continuous(limits = c(-25, 400), breaks = seq(-25, 400, by =25))+
  scale_x_discrete(breaks = seq(0, 60, by = 4)) +
      theme (legend.position = "top", 
         legend.direction = "horizontal", 
              axis.text = element_text(color = "black", size=10), 
...
lm_mi <-lmer(as.numeric(MotionIndex) ~TRT*timecat + AgeatEnroll + Egroup + (1|Cow), data=hourrm)
summary(lm_mi)
...
trt_compare <- emmeans(lm_mi, pairwise ~ TRT, adjust = "tukey")
trt_compare$contrasts


emm_mi <- emmeans(lm_mi, ~ TRT | timecat)
contrast_results <- pairs(emm_mi, reverse = FALSE)
summary(contrast_results, adjust = "tukey")
...
MI_data <- data.frame(
  Period = factor(rep(c("Day 1", "Day 2", "Day 3"), each = 3), 
                  levels = c("Day 1", "Day 2", "Day 3")),
  Treatment = rep(c("BO", "B+N", "LIDO"), times = 3),
  MeanTime = c(
    # Day 1
    84.3, 62.0, 67.5,
    # Day 2
...
custom_colors <- c("BO" = "red4", "B+N" = "goldenrod2", "LIDO" = "wheat4")
ggplot(MI_data, aes(x = Period, y = MeanTime, group = Treatment, color = Treatment)) +
  theme_classic () +
  geom_line(size = 1) +   
  scale_color_manual(values=custom_colors)+
  # Draw lines
  geom_point(size = 1) +                         # Add points at each data node
  ...
  theme(text=element_text(size=12, family = "Calibri"))# Adjust Y-axis for better visibility
```
